...
buffer_t rlp::RLPDecoder::Decode(const buffer_t& bytes_input) {
    ...
    if(first_byte_ <= SHORT_STRING) {
        ...
        decoded_ = utils::Slice(bytes_input, 1, length_);
        ...
        decoded_ = utils::Slice(bytes_input, l_length_ + 1, length_ + 1); // + 1 for first byte length
    } else if(first_byte_ <= 0xf7) {
        // a list between 0-55 bytes long
        length_ = first_byte_ - 0xbf;
        auto inner_reminder = utils::Slice(bytes_input, 1, length_);
...
std::string rlp::RLPDecoder::DecodeString(const std::string& input) {
    ...
    const buffer_t byte_input_ = utils::ToBytes(input);
